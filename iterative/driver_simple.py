...
if __name__ == "__main__":
    ...
    for M in Mvals:
        ...
        for niters in range(maxiters):
            ...
            if (np.linalg.norm(x - x0)/np.linalg.norm(x0+1e-12) < tol):
                break
            x0 = x.copy()
        toc = time.perf_counter()
        print("  Jacobi:    niters = %5d, time = %.1e, error = %.1e, residual = %.1e" 
              % (niters+1, toc-tic, np.linalg.norm(x - xtrue), np.linalg.norm(b - A@x)))
        ...
        print("  GS:        niters = %5d, time = %.1e, error = %.1e, residual = %.1e" 
              % (niters+1, toc-tic, np.linalg.norm(x - xtrue)/np.linalg.norm(xtrue), 
                 np.linalg.norm(b - A@x)/np.linalg.norm(b)))
        ...
        print("  Sym-GS:    niters = %5d, time = %.1e, error = %.1e, residual = %.1e" 
              % (2*(niters+1), toc-tic, np.linalg.norm(x - xtrue)/np.linalg.norm(xtrue), 
                 np.linalg.norm(b - A@x)/np.linalg.norm(b)))
        ...
        for omega in omega_vals:
            ...
            print("  SOR(%.2f): niters = %5d, time = %.1e, error = %.1e, residual = %.1e" 
                  % (omega, niters+1, toc-tic, np.linalg.norm(x - xtrue)/np.linalg.norm(xtrue), 
                     np.linalg.norm(b - A@x)/np.linalg.norm(b)))
